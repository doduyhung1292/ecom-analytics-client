...
<script>
  ...
  if (values.username && values.password) {
        await axios.post('http://localhost:8080/account/login', {
          ...
          if (res.data.statusCode == 1) {
            updateUserInformation(res.data.user);
            asyncSetUserRole();
  ...
  const asyncSetUserRole = function() {
    store.dispatch('asyncSetUser');
  }
  const updateUserInformation = function(dataUser) {
    store.dispatch('addToUserInformation', dataUser)
  }
</script>
